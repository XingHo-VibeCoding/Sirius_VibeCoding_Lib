...
| **一次性把 3D 场景全建好** | 会拖慢首屏，可能违反 V1「3 秒内看到画面」。所以 3D 库**按需加载**：先给 2D 主图，点开天体时才加载 3D |
...
| 配置项 | 作用 | 默认值（示意） |
|---|---|---|
| `TIME_SCALE_OPTIONS` | 时间流速的可选档位 | 暂停 / 实时 / 加速若干档 |
| `DEFAULT_TIME_SCALE` | 打开时的默认流速 | 实时 |
| `SCALE_MODE_DEFAULT` | 默认尺度模式 | 图示比例 |
| `GUIDE_ENABLED` | 引导词是否默认开启 | 开启 |
...
| # | 问题 | 何时解决 |
|---|---|---|
| 1 | **哈雷彗星的数据来源**（不在 NASA Planetary Fact Sheet 覆盖范围内） | **Day 6 开始编码前必须确认** |
